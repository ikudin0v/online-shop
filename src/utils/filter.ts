export function filter(initialList: any[], initialFilterItems: (string | undefined)[], filteredList: any[], filterItems: (string | undefined)[], newItem: string) {
	if (newItem === "clear") {
		...
		if (filterItems.indexOf(newItem) !== -1 && filterItems.length !== initialFilterItems.length) {
			// Добавление или удаление цвета?
			...
			// добавляем цвет в список фильтрации
			if (filterItems.length === initialFilterItems.length) {
				filterItems = []
			} // если фильтр сброшен *(отображается всё), то обнулить список фильтров и потом добавлять уже новый
