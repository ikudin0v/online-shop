		...
		<div className="container d-flex flex-column mt-3">
			...
			{Object.keys(currentUser.orders).map((order) => (
				<div className="card border border-0 shadow p-3 mb-3 bg-body-tertiary rounded" key={order}>
					...
					<div>
						{"Итого: " +
							Object.keys(currentUser.orders[order]).reduce((totalmem, item) => currentUser.orders[order][item].quantity + totalmem, 0) +
							" " +
							getNameByQuantity(Object.keys(currentUser.orders[order]).reduce((totalmem, item) => currentUser.orders[order][item].quantity + totalmem, 0)) +
							" на сумму " +
							Object.keys(currentUser.orders[order]).reduce((totalPrice, item) => currentUser.orders[order][item].product.price * currentUser.orders[order][item].quantity + totalPrice, 0) +
							" Р"}
					</div>
				</div>
			))}
