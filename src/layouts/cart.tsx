...
const CartPage = () => {
	...
	const totalCost:number = Object.keys(currentUser.cart).reduce((partialSum, item) => partialSum + currentUser.cart[item].product.price * currentUser.cart[item].quantity, 0)
	const totalItems:number = Object.keys(currentUser.cart).reduce((partialSum, item) => partialSum + currentUser.cart[item].quantity, 0)
	...
	async function makeOrder() {
		...
		setCurrentUser({...currentUser, cart:{}, orders:{...currentUser.orders, [data.name]:localStorageService.getCart()}})
